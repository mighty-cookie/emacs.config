...
(custom-set-variables
 ...
 '(c-default-style
   '((c-mode . "bsd") (c++-mode . "bsd") (java-mode . "java")
 ...
 '(mouse-wheel-scroll-amount
   '(3 ((shift) . hscroll) ((meta)) ((control meta) . global-text-scale)
       ((control) . text-scale)))
 '(org-agenda-files
   '("c:/Users/jure.pelhan/Desktop/TODO.org"
     "c:/source/lfs-diff-viewer/TODO.org"))
 '(org-confirm-babel-evaluate nil)
 '(package-selected-packages
   '(auctex elpy expand-region flycheck ivy magit marginalia move-text
            multiple-cursors projectile rainbow-delimiters undo-tree))
...
(custom-set-faces
 ...
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(default ((t (:family "Cascadia Code" :foundry "outline" :slant normal :weight regular :height 98 :width normal))))
...
(use-package flycheck
  :ensure t
  :custom
  (flycheck-python-pycompile-executable "C:\\ProgramData\\anaconda3\\python.exe")
  :init (global-flycheck-mode))
;;
;; AucTex
(use-package auctex
  :ensure t
  :custom (setq TeX-auto-save t)
...
;;
(defun bespoke/mark-embraced-outer (n)
  (interactive "P")
  (if (not (consp n))
      (let ((n (prefix-numeric-value n)))
        (if (> n 0)
            (progn
              (backward-up-list n t t)
              (push-mark (point))
              (forward-sexp)
              (activate-mark))
          ;; kill if negative argument has
          ;; been providedp
          (backward-up-list (abs n) t t)
          (kill-sexp)
          )
        )
    (let ((n (log (prefix-numeric-value n) 4)))
      (progn
        (down-list n t)
        (backward-char)
        (push-mark (point))
...
;;
(defun bespoke/mark-embraced-inner (n)
  (interactive "P")
  (if (not (consp n))
      (let ((n (prefix-numeric-value n)))
        (progn
